  ...
  <view class="container">
    <view class="context">
      <page-main :status="status" :refresherTriggered="refresherTriggered"
        ...
        emtpyMessage="没有收到目击或上报情况"
      >
        <view class="cat-content">
          <footprint-item v-for="item in items" :key="item.footprint.id" :item="item"/>
        </view>
      </page-main>
    </view>
  </view>
...
import FootprintItem from '@/components/footprint-item/footprint-item.vue'
import { Footprints } from '@/api'

@Component({
  name: 'FootprintsManage',
  components: { FootprintItem }
})
export default class FootprintsManage extends Vue {
  // 是否初始化
  ...
  async loadData (isRefresh?: boolean) {
    ...
    this.isLoading = true
    const items = await Footprints.list({
